...
Current security network solutions neglect  the internal monitoring of users activity, as they assume that inner user interaction is secure and will not harm any asset of the network [1]. However the lack of supervision and monitoring arise two new kind of network vulnerabilities: 1) an inner user attacks the network with full knowledge of what he is doing, while in the other hand 2) an inner user, is victim of an attacker, unconsciously causing attacks to the network, or networks, where it has any activity. In both scenarios, the user is taking advantage of its unsupervised position.
...
Basic malicious users are forced to explode known system vulnerabilities, depending on missing security patches in the operating systems and to use code (snippets and scripts) written by experienced users, as they have a lack of technical formation, making its network behavior similar to a scriptkiddie. On the other hand, intermediate-advance users may use new specialized software or tools to break into the network and damage valuable information.

In the second case where the host has been compromised, and it's being used to generate an attack, identifying behaviors among basic users or advanced is more difficult, as the computer is only used as a gateway.

Identifying threats and attacks has been a mayor problem in security addressed in different ways, solutions are not completely error free as human factor intervenes and attackers work on developing new and innovative techniques. User identification could help to improve current network solutions decreasing false-positives rates, training and processing times, and operations costs as it increases accuracy.
...
The daily interactions of our users will define what is a normal behaviour inside the network, maybe on an ethical hacker consultancy, or a security class, network attacks will be common and normal, but this will not be the situation in a house network or in the human resources department. To define what is normal in our environment is vital.
...
Another approach for profiling traffic behavior is used by Kuai [11] and Qin[12], in which clusters of information are created based on the used communication protocols.

Kuai [11], identifies and analyzes clusters of hosts or applications that exhibit similar communication patterns. Network traffic is modeled by bipartite graphs to obtain one-mode projections. Projections are used to represent source host(s) that communicate with the same destination host(s) and to connect destination hosts that communicate with the same source host(s). Obtained One-mode projection graphs are used as base for building similarity matrices of internet end-host. Simple spectral clustering algorithm is applied over the matrices to discover inherent end-host behavior cluster. 

Kuai [11] concluded that it was possible to detect anomalous traffic behavior through network traffic prefixes, as the end hosts tend to stay in the same cluster over time, and the number of behavior cluster was not correlated to the number of observed hosts.

For Qin's implementation [12] the algorithm is implemented using the traffic at port 80 (HTTP protocol) and integrating the destination URL, not only the IP address. As Kuai, Qin also concluded that at least 93\% of the hosts remain on the same behavior cluster.
...
Supervised learning algorithms make decisions based on a previous identified set of examples (labeled information) [9].
...
Support vector machines algorithm is proposed by Vapnik [16], in which binary classification is performed. The algorithm checks the complete dataset for the optimal separating hyperplane between two classes by maximizing the margin between the classes closest points. Points lying on the boundaries are called support vectors, and the middle of the margin is our optimal separating hyperplane[17].

\paragraph{Artificial neuronal networks (ANN)} % level 4
In 1943, Warren S. and McCulloh, developed the first conceptual model of an artificial neural network [18] in which they describe the concept of a neuron as a single living cell in a network of cell that receives inputs, processes those inputs and generates an output.

Today, the basic structure of an artificial neural network involves a network of manny interconnected neurons, in which each neuron act as a simple processing element that individually handles a part of a bigger problem. A neuron computes an output using an activation function, that considers the weighted sum of its inputs [19].
...
Genetic Algorithms were invented to mimic some of the processes in natural evolution. It is known to be an ideal technique for finding solutions for optimization problems [20] [21]. To apply this process of evolution to the selected problem, it is needed to define three basic elements: individual gene presentation and initialization, evaluation function model, and a specific function of genetic operators and their parameters [23]. In each of them we will define the characteristics of our individuals, how they will be evaluated and selected as the best elements of the population and how they will be mixed among them respectively.
...
The idea behind hybrid classifiers is to combine several machine learning techniques, so that the system performance can be significantly improved. More specifically, a hybrid approach typically consists of two functional components, in order to complement each other. An example of this is when cluster algorithms such as $k$-neighbor, Self organizing maps, or SVM are used in the first part of the process as optimization of the training model and the second part of the process is in charge of the prediction. [15]

\subsection{Self-organizing Maps} % level 2
The Self-Organizing Map algorithm performs a nonlinear, ordered, and smooth mapping of high-dimensional input data manifolds onto the elements of a regular, low-dimensional array [23]. The algorithm converts non-linear statistical relationships between data elements in a high-dimensional space into geometrical relationships between elements in a two-dimensional map (lattice), called the Self-Organizing Map (SOM)[24]. A SOM can then be used to visualize the clusters, of an input space. Each element at SOM is a neuron, and is a representation of a multidimensional vector with a cartographic position denoted with x and y. If elements in the input space are characterized using $k$ parameters and represented by $k$-dimensional vectors, each neuron in the SOM lattice is also specified as $k$-dimensional vector.
...
During SOM training each time a BMU is found, it's weight and the neurons in its vicinity get their weights vectors updated according to the features updated rule [22].

\begin{equation}
N(t + 1) = N(t) + \omega h(r,s)(v(t) - N(t))
\end{equation}
Where $\omega$ is the learning step width and its value is a constant $(0 < \omega \ll 1)$. The function $h(r,s)$ is the neighborhood function, which determines the size and the nature of the neighborhood around the BMU. $h(r,s)$ is illustrated in Figure 1. For most applications it has the maximum value of one when $s = r$ and decreases when the distance between $s$ and $r$ increment.

\begin{figure}[H]
	\centering
		\includegraphics[scale=.25]{som}
		\caption{SOM neighborhood radius}
\end{figure}
...
This layer takes care of the verification that bit sequences are passed correctly between two nodes, If errors occur, this level request the retransmission of the corrupted sequence, providing the upper layers an error free transmission over the link. To do this, data link layer provides: a) link control, in which logical establishment and termination between nodes is done, b) frame control, which defines the transmission rate, establish the sequence of the received data, verifies data integrity and c) determines when the node is allowed to use the physical medium. Common protocols of this layer are: HDCL for point to point and point to multipoint connections, and IEEE 802 for local networks [27]. 

\paragraph{Network layer} % level 4
Network layer role is to deliver a packet form one logical address (in the case of an IP, an IP) to another. Router devices use the generated frames to send packets through the network, depending on several key field of the datagram header such as: the source and destination IP address, Time To Live counter (TTL), and the checksum [28].

\paragraph{Transport layer} % level 4
Transport layer ensures the reliable arrival of messages and provides error checking mechanisms and data flow controls. The Transport layer provides services for both connection-mode transmissions and for connectionless-mode transmissions. Two transport protocols are used over the internet: TCP [29] and UDP [30]. TCP ensures the delivery of the packages due the implementation of re transmission mechanisms such as error checking and numbering for check the sequence of the packages, however UDP is more often used for network transmissions where the lost of any package is not critical.

\paragraph{Session layer} % level 4
Session layer facilitates the exchange of data between applications, by organizing and structuring the the processes among them. The basic unit of this layer is the protocol data unit (PDU) [27].

\paragraph{Presentation layer} % level 4
The presentation layer is in charge of formatting the data to be presented to the application layer. this layer acts as a translator, changing from a custom format used by the application to a common format used in the receiver station [27].
...
For network data capture phase, a set of raw packages is obtained for each user through tcpdump library.
...
Using TCP package as the working unit is not possible due the great volume of packages, and time consuming for processing each one individually [33].

Instead, package sets $[\beta,\gamma,\phi]$ are individually processed and turned into chunks. The obtained chunks are the the working units and conform the different data sets, Build data set $\beta_1$, the Train data set $\gamma_1$ and the Evaluation data set $\phi_1$ respectively, for better understanding check Figure 4.

\begin{figure}[H]
	\centering
		\includegraphics[scale=0.2]{fig-three}
		\caption{Data set module}
\end{figure}
...
To define the the size $n\times n$ of the lattice, ten experiments were evaluated with 50, 75, 100, 125 and 150 neurons. Figure 5. Shows the average of the results by user.
...
The train module implements the SOM-based approach for user characterization. In the training phase 10 epochs are used to process the lattice. After this phase, an user network behavior pattern is obtained. This pattern may differ between patterns obtained from the same user, because of the random selection of the build elements. Figure 6 shows lattice training during it's initial phases until it get's completely trained.

\begin{figure}[H]
	\centering
		\includegraphics[scale=0.2]{fig-training}
		\caption{Pattern creation}
\end{figure}
...
Evaluation is done by showing each element of the organization evaluation data set to the organizational pattern, once every element is evaluated the SOM lattice is processed in order to identify which neurons in the SOM lattice correctly match to the vector $V_n$ in the organizational dataset. As we already know the origin of the vector $V_n$, is easy to find out if the match is correct or not. Figure 7. shows how an organizational pattern is conformed and how it is evaluated.

\begin{figure}[H]
	\centering
		\includegraphics[scale=0.3]{evaluation}
		\caption{Pattern evaluation}
\end{figure}
...
\multicolumn{1}{|l|}{\cellcolor[HTML]{9B9B9B}\textbf{}}                  & \textbf{CABD}             & \textbf{CABE}             & \textbf{CADE}             & \textbf{CBDE}             \\ \hline
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Correct user A}}   & 66.6                      & 62.52                     & 60.91                     & \cellcolor[HTML]{EFEFEF}- \\ \hline
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Incorrect user A}} & 33.4                      & 37.48                     & 39.09                     & \cellcolor[HTML]{EFEFEF}- \\ \hline
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Correct user B}}   & 66.54                     & 68.32                     & \cellcolor[HTML]{EFEFEF}- & 74.54                     \\ \hline
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Incorrect user B}} & 33.46                     & 31.68                     & \cellcolor[HTML]{EFEFEF}- & 25.46                     \\ \hline
\rowcolor[HTML]{EFEFEF} 
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Correct user C}}   & Idle                      & Idle                      & Idle                      & Idle                      \\ \hline
\rowcolor[HTML]{EFEFEF} 
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Incorrect user C}} & Idle                      & Idle                      & Idle                      & Idle                      \\ \hline
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Correct user D}}   & 14.9                      & \cellcolor[HTML]{EFEFEF}- & 20.98                     & 80.71                     \\ \hline
\cellcolor[HTML]{C0C0C0}{\color[HTML]{000000} \textbf{Incorrect user D}} & 85.1                      & \cellcolor[HTML]{EFEFEF}- & 79.02                     & 19.29                     \\ \hline
\cellcolor[HTML]{C0C0C0}\textbf{Correct user E}                          & \cellcolor[HTML]{EFEFEF}- & 37.85                     & 53.48                     & 70.18                     \\ \hline
\cellcolor[HTML]{C0C0C0}\textbf{Incorrect user E}                        & \cellcolor[HTML]{EFEFEF}- & 62.15                     & 46.52                     & 29.82                     \\ \hline
\end{tabular}
\caption{Results}
\end{table}

Table 1 shows the average of results of every user. It is possible to see that results are constant over the experiments, having an average of 60\% of correct matching.

User A and D are the better results that were obtained over the experiments. User A maintains the percentage of correct and incorrect matches over the different experiments (CABD, CABE, CADE), also  when the SOM lattice dimensions change. On the other hand user D is not completely stable but gives a lot of information when is compared against other users.

User D and B present a particular pattern when the SOM lattice dimensions change, when SOM lattices are build with 50, 100, and 125 neurons the results show a low rate of correct matches between 40\% and 50\%, but lattices build with 75,125 and 150 have a high rate of positive matches  between 65\% and 80\%, showing that these two users may have a similar behavior, but further study is needed to understand why the results vary constantly with different SOM lattices dimensions.

When user A and D are compared, experiment CBDE for user D, is completely different from CABD and CADE where correct matches are 14.9\% and 20.98\% respectively and CBDE results show a raise to 80.71\%, suggesting that, when user A is evaluated in the same pattern along with user D, both behaviors are mixed, giving an incorrect match. Similar behavior could denote the use of the same services, spend time in the computer or similar usage schedules.

To address this finding, experiments with different dimension of SOM lattices were performed avoiding comparing users A and D at the same time. Figure. 8 shows results of user A and D respectively.

\begin{figure}[H]
	\centering
		\includegraphics[scale=0.75]{fig-four} 
		\caption{User A and user D results comparison}
\end{figure}

For user A, the results raise from a correct average of 63.34\% to a 73.98\% of correct match, instead for user D, the graphic shows that results are stable and constant, with a raise from 38\% to 82.66\%.

User E results are not as constant as desired, because experiments CABE, CADE, and CBDE get different results each, but do not suggest any explanation of why this is happening, opening some questions like, if more experiments are needed for this user? or if the used datasets haver enough quality to be analyzed?

\section{Conclusions} % level 1
As defined previously, [1] has found a security breach addressed by the inner users of a network due the lack of supervision. Current security systems are not able to detect this behavior as they are focusing on outside attacks and most of the time, with outdated patterns or attack firms, making very difficult to detect novel attacks that come from outside of the network and even worse for inner attacks.
Updates information for detecting novel attacks has a high cost or is not available, creating the need of systems that can be updated continuously but without the need of a human supervision.
This work shows an implementation of SOM algorithm, results show that based on three metrics: a) TCP-UDP, b) Internal IP metric and c) Web traffic metric, an user can be identified among others with a rate between 60\% and 70\%. SOM algorithm works based on three datasets, that are directly obtained from  user's network traffic.
Evaluated users were part of the same department in college, however results show that when users of the same area were excluded in the evaluation results could get to 90\% of correct matching, indicating that further work is needed for selecting features to evaluate.

\subsection{Future work} % level 2
The automatic creation of a dataset which defines correctly what is the normal behavior of an user inside the network would be very helpful for organizations, since they will be capable of maintain their networks secure, at a low cost and with a very low intervention of the human factor. This works gives a approach of how this could be accomplished in three stages: 1) network data capture, 2) data processing and 3) pattern evaluation.

The main improvements could be done in the data processing stage, mainly in three big areas: 1) Feature selection, 2) SOM algorithm implementation and experimentation and 3) Dataset improvement.

\subsubsection{Feature selection} % level 3
In this work, three main features are evaluated, however [24] combines different approaches, which are also obtained from the web traffic of the user, as conversation time, specific used services and connection times, obtaining more data to process and submit into the SOM lattice, to improve user characterization.

\subsubsection{SOM algorithm implementation and experimentation} % level 3
Due reduced computational power, SOM algorithm was executed only with 25 epochs in the training phase, giving correct matches, however scaling to 1000 epochs could improve cluster definition which will define better user characteristics and idle behavior. SOM algorithm was implemented using a square lattice and a circular neighborhood function, using a different form of lattice and neighborhood function, could improve the training processing time and the convergence of the algorithm.

\subsubsection{Dataset improvement} % level 3
As defined in Section 4.2.1, datasets are created from at least two weeks of network traffic capture, however it can be seen that there is missing data, due  error connections, errors on host program or other not known problems. Improving the data capture could help to define better user behavior.
The selected evaluated users, all they belong to one single department of the college, making more difficult to identify them. Capturing network traffic of different departments and compare them against each other could give new results and probably raise the correct matches results.

\section{Bibliography} % level 1

[1] CISCO, "Annual Security Report," 2015.

[2] K. A. Jalil, M. H. Kamarudin, and M. N. Masrek, "Comparison of Machine Learning algorithms performance in detecting network intrusion," 2010 International Conference on Networking and Information Technology, 2010.

[3] O. Al-Jarrah, A. Siddiqui, M. Elsalamouny, P. Yoo, S. Muhaidat, and K. Kim, "Machine-Learning-Based Feature Selection Techniques for Large-Scale Network Intrusion Detection," 2014 IEEE 34th International Conference on Distributed Computing Systems Workshops, 2014.

[4] W. Tian and J. Liu, "A new network intrusion detection identification model research," 2010 2nd International Asia Conference on Informatics in Control, Automation and Robotics (CAR 2010), 2010.

[5] L. P., E. E., and S. S., "Intrusion detection with unlabeled data using clustering," In Proceedings of ACM CSS Workshop on Data Mining Applied to Security, p. 58.

[6] A. Lakhina, M. Crovella, and C. Diot, "Diagnosing network-wide traffic anomalies," ACM SIGCOMM Computer Communication Review, vol. 34, no. 4, p. 219, Jan. 2004.

[7] S. Staniford, J. A. Hoagland, and J. M. Mcalerney, "Practical automated detection of stealthy portscans," Journal of Computer Security, vol. 10, no. 1-2, pp. 105-136, Jan. 2002.

[8] J. Marin, D. Ragsdale, and J. Sirdu, "A hybrid approach to the profile creation and intrusion detection," Proceedings DARPA Information Survivability Conference and Exposition II. DISCEX01.

[9] D. Anderson, T. F. Lunt, and H. Javitz, Detecting unusual program behavior using the statistical component of the next-generation intrusion detection expert systems (NIDES). Menlo Park: SRI International. Computer Science Laboratory, 1995.

[10] A. Badea, V. Croitoru, and D. Gheorghica, "Computer networks security based on the detection of users behavior," 2015 9th International Symposium on Advanced Topics in Electrical Engineering (ATEE), 2015.

[11] K. Xu, F. Wang, and L. Gu, "Behavior Analysis of Internet Traffic via Bipartite Graphs and One-Mode Projections," IEEE/ACM Transactions on Networking, vol. 22, no. 3, pp. 931-942, 2014.

[12] T. Qin, X. Guan, C. Wang, and Z. Liu, "MUCM: Multilevel User Cluster Mining Based on Behavior Profiles for Network Monitoring," IEEE Systems Journal, vol. 9, no. 4, pp. 1322-1333, 2015.

[13] K. L. Priddy, Artificial neural networks: an introduction. New Delhi: Prentice-Hall India, 2007.

[14] W. K. Q, "Distance metric learning for large margin nearest neighbor classification," Journal of Machine Learning Research, pp. 207-244, Feb. 2009.

[15] Z. Muda, W. Yassin, M. N. Sulaiman, and N. I. Udzir, "Intrusion detection based on k-means clustering and OneR classification," 2011 7th International Conference on Information Assurance and Security (IAS), 2011.

[16] V. V., Statistical learning theory (Vol. 1). Wiley, New York, 1998.

[17] H. M. A and D. S. T, "Support vector machines," IEEE Intelligent Systems and their applications, pp. 18-28, 1998.

[18] G. Palm, "Warren McCulloch and Walter Pitts: A Logical Calculus of the Ideas Immanent in Nervous Activity," Brain Theory, pp. 229-230, 1986.

[19] B. M. C. C. Brio, A. S. Molina, "Redes neuronales y sistemas difusos," in Redes neuronales y sistemas difusos, México, D.F.: Alfaomega, 2002.

[20] Y. Rahmat-Samii, E. Michielssen, "Electromagnetic optimization by genetic algorithms," in Electromagnetic optimization by genetic algorithms, New York: John Wiley \& Sons, 1999.

[21] C. R. Reeves, "A genetic algorithm for flowshop sequencing," Computers \& Operations Research, vol. 22, no. 1, pp. 5-13, 1995.

[22] T. Shon and J. Moon, "A hybrid machine learning approach to network anomaly detection," Information Sciences, vol. 177, no. 18, pp. 3799-3821, 2007.

[23] T. Kohonen. "Self Organizing Maps,". Springer, third edition, 2001.

[24] M. Ramadas and M. Ramadas, "Detecting anomalous network traffic with self-organizing maps," in Detecting anomalous network traffic with self-organizing maps, 2003.

[25] W. Stallings and W. Stallings, "Data and computer communications," in Data and computer communications, Harlow: Pearson, 2015.

[26] "Ccnp Complete Study Guide Exams 642-801; 642-811; 642-821; 642-831," in Ccnp Complete Study Guide Exams 642-801; 642-811; 642-821; 642-831, Sybex Inc, 2006.

[27] "OSI Reference Model - The ISO Model of Architecture for Open Systems Interconnection," The Best of the Best, 2009.

[28] J. F. and K. W., Computer networking: a top-down approach (Vol. 4). Boston, Massachusetts, 2009.

[29] "RFC-es - Grupo de Traducción de RFC al español," RFC-es - Grupo de Traducción de RFC al español. [Online]. Available: https://www.rfc-es.org/rfc/rfc0793-es.txt. [Accessed: 24-Aug-2017].

[30] "Internet Engineering Task Force (IETF)," Internet Engineering Task Force (IETF). [Online]. Available: https://www.ietf.org/rfc/rfc768.txt. [Accessed: 24-Aug-2017].

[31] B. Guttman, E. Roback, "An introduction to computer security, the NIST handbook," in An introduction to computer security, the NIST handbook, Gaithersburg, MD: U.S. Dept. of Commerce, Technology Administration, National Institute of Standards and Technology, 1995.

[32] K. Kent, K. Scarfone, P. Mell, "Guide to intrusion detection and prevention systems (IDPS): recommendations of the National Institute of Standards and Technology," in Guide to intrusion detection and prevention systems (IDPS): recommendations of the National Institute of Standards and Technology, Gaithersburg, MD: U.S. Dept. of Commerce, Technology Administration, National Institute of Standards and Technology, 2014.

[33] A. Lakhina, M. Crovella, and C. Diot, "Characterization of network-wide anomalies in traffic flows," Proceedings of the 4th ACM SIGCOMM conference on Internet measurement, pp. 201-206, Oct. 2004.
...
%[5] Dozono, H., Itou, S., and Nakakuni, M. (2007). Comparison of the adaptive authentication systems for behavior biometrics using the variations of self organizing maps. International 
%Journal of Computers and Communications, 1(4), 108-116.

%[8] L. Zhe, S. Weiqing and W. Lingfeng, "A neural network based distributed intrusion detection system on cloud platform," IEEE 2nd International Conference on Cloud Computing and Intelligence Systems, vol. 1, pp. 75-79, 2012.

%[18] Tsai, C. F., Hsu, Y. F., Lin, C. Y., and Lin, W. Y. (2009). Intrusion detection by machine learning: A review. Expert Systems with Applications, 36(10), 11994-12000.

%[19] Michalski, R. S., Bratko, I., and Bratko, A. (1998). Machine learning and data mining; methods and applications. John Wiley and Sons, Inc.

%[20] Theodoridis, S., and Koutroumbas, K. (2006). Clustering: basic concepts. Pattern Recognition, 483-516.s

%[30] https://www.symantec.com/connect/articles/network-intrusion-detection-signatures-part-one

%[31] http://www.ciscopress.com/articles/article.asp?p=1336425
