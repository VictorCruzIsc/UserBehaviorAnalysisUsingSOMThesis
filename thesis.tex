...
La necesidad de la creacion de un dataset efectivo y automatizado
Porque es bueno SOM y porque puede apoyar a resolver esta problematica
Discusion de los resultados
Although many of the results do not get over 70\% of correct matching, we think think that is possible to identify an user over the network based in it's network traffic. User A and D, have given valuable information since users in real life have similar behaviors. We also think that mismatching among users is because all them work in the same area and belong to the same school department, having a high probabilities that they share many of their activities.
...
The main improvements could be done in the data processing stage, mainly in two big areas: 1) Neurons feature vectors $V_n$ , in this work, three main features are evaluated, however [1] combines different approaches, which are also obtained from the web traffic of the user, as conversation time, specific used services and connection times, giving him more data to process into the SOM lattice and receive useful feedback from the algorithm, to improve user characterization. the second area. 2) The second opportunity area is related to the SOM algorithm implementation and experimentation, due reduced computational power, SOM algorithm was executed only with 25 epochs in the training phase, giving correct. Scaling to 1000 epochs could improve cluster definition which will define better user characteristics and idle behavior. In the same line SOM algorithm was implemented using a square lattice and a circular neighborhood function, using a different form of lattice and neighborhood function, could improve the training processing time and the convergence of the algorithm.
