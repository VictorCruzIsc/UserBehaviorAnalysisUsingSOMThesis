...
As defined previously, [10] has found a security breach addressed by the inner users of a network due the lack of supervision. Current security systems are not able to detect this behavior as they are focusing on outside attacks and most of the time, with outdated patterns or attack firms, making very difficult to detect novel attacks that come from outside of the network and even worse for inner attacks.
...
In this work, three main features are evaluated, however [1] combines different approaches, which are also obtained from the web traffic of the user, as conversation time, specific used services and connection times, obtaining more data to process and submit into the SOM lattice, to improve user characterization.
