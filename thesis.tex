...
When user A and D are compared, experiment CBDE for user D, is completely different from CABD and CADE, because correct matches raise to 80.71\%, suggesting that when user A is evaluated in the same pattern the behaviors are getting confused as they may have a similar behavior, in used services, time spend in the computer or usage schedules.

To address this finding, experiments with different dimension of SOM lattices were performed avoiding comparing users A and D. Table 2 and 3 show results of user A and D respectively. 


As it can be seen the correct match raises from 60\% to almost 80% misma compracion para el otro usuario, mete los datos de la desviacion standar.
