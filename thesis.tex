...
Identifying network attacks has been addressed in two different ways: 1) Signature Based Intrusion Detection Systems, known attack firms are provided to security network devices such as firewalls, antivirus or intrusion detection systems to check if the current network traffic matches any of them and 2) Anomalies-based Intrusion Detection Systems, use the artificial intelligence or machine learning algorithms to detect anomalous behavior.
...
In Anomalies-based IDS, in order to avoid firms classification which implies spending time and human resources, firms are obtained from general datasets such as KDCUPP99 or ANOTHERDATASET [REFERENCES, XX, XX, XX], causing a big rate of false-positives, making the system not as reliable as it should be.
...
As explained on sections 1.2 and 1.3, there's an obvious need to improve current network security tools, addressing as many problems as possible to solve. By the time we were doing this work we think that, identifying network attacks could be much easier if we reduce variables to the equation. The actual network security systems need to deal, with normal and anomalous traffic inside and outside the network, mainly we cannot control what is outside the frontiers of our network, but we can control what is happening inside ours, and define what is a normal behavior in our environment.

The behavior of our users, will define what is normal inside the network, maybe on an ethical \textit{hacker} consultancy, or a security class, network attacks will be common and normal, but this will not be the situation of a house network or a human resources department. Define what is normal in our environment is vital.
...
In this work we demonstrate that it is possible to identify a network user, with a high rate detection, based on it's processed network traffic, and that it is possible to differentiate it from other users in the same network.
...
As SOM algorithm does not need, specific labeled dataset for training or evaluation, adding new users or departments to the organization is as easy as adding what the hosts are transmitting in the correct format. Such tasks will automated and do not need the supervision of the network administrator.
